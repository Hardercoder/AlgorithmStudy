...
import Foundation
// MARK: 定义：树节点
public class TreeNode: CustomStringConvertible, Hashable {
...
// MARK: 定义：链表节点
public class ListNode: Hashable, CustomStringConvertible {
    public var description: String {
        var str = "\(self.val)"
        var node = self.next
        // 最多打印100个节点
        var deepTh = 0
        while node != nil && deepTh < 100 {
            str += " -> \(node!.val)"
            node = node!.next
            deepTh += 1
        }
