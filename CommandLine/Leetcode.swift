...
class Leetcode {
    ...
    func lc_21() {
        ...
        func mergeTwoLists(_ l1: ListNode?,
            ...
            return Cyc2018_swordOffer.Cyc2018_swordOffer_LinkList().merge(l1, l2)
        }
    }
    func lc_26() {
        // 26. 删除有序数组中的重复项
    ...
    func 剑指Offer58I() {
        ...
        func reverseWords(_ s: String) -> String {
            return Algorithm4Review().reverseWords(s: s)!
        }
        
        func mine(_ s: String) -> String {
            return Cyc2018_swordOffer.Cyc2018_swordOffer_DoublePointer().reverseSentence(s)
        }
    ...
    func 剑指Offer58II() {
        ...
        func reverseLeftWords(_ s: String, _ n: Int) -> String {
            return Cyc2018_swordOffer.Cyc2018_swordOffer_DoublePointer().leftRotateString(s, n)
        }
    }
    func 剑指Offer06() {
        ...
        func reversePrint(_ head: ListNode?) -> [Int] {
            return Cyc2018_swordOffer.Cyc2018_swordOffer_LinkList().reversePrint(head)
        }
    ...
    func lc_5() {
        ...
        func mine(_ s: String) -> String {
            ...
            func isPalindrome(_ i: Int,
                              _ j: Int) -> Int {
                var l = i, r = j
                while l >= 0 && r < sCharsCount && sChars[l] == sChars[r] {
                    l -= 1
                    r += 1
                }
                return r - l - 1
            }
            for i in 0..<sCharsCount {
            ...
            return String(sChars[start..<(end+2)])
        }
    ...
    func 剑指Offer26() {
        ...
        func isSubStructure(_ A: TreeNode?, _ B: TreeNode?) -> Bool {
            return Cyc2018_leetcode.DataStructure_Tree().isSubtree(A, B)
        }
    }
    func lc_226() {
        // 226. 翻转二叉树
        ...
        func invertTree(_ root: TreeNode?) -> TreeNode? {
            return Cyc2018_leetcode.DataStructure_Tree().invertTree(root)
        }
    }
    ...
    func lc_101() {
        ...
        func isSymmetric(_ root: TreeNode?) -> Bool {
            return Cyc2018_leetcode.DataStructure_Tree().isSymmetric(root)
        }
    }
    ...
    func lc_8() {
        ...
        func myAtoi(_ s: String) -> Int {
            ...
            // 判断是否是负数
            let isNegatvive = str.first! == "-"
            // 是否应该移除第一个字符
            let shouldDropFirst = str.first! == "-" || str.first! == "+"
            // 过滤掉不能转换成整数的字符
            let validStr = shouldDropFirst ? String(str.dropFirst()) : str
            let defInt = -1
            let strInts = validStr.map { Int(String($0)) ?? defInt }
            let maxDiv = Int32.max / 10,
                maxRemind = Int32.max % 10,
                minDiv = Int32.min / 10,
                minRemind = Int32.min % 10
            var res = 0
            ...
            for int in strInts {
                ...
                res = res * 10 + (isNegatvive ? int * -1 : int)
            }
            return res
        }
    ...
    func findTheWinner(_ n: Int, _ k: Int) -> Int {
        return Cyc2018_swordOffer.Cyc2018_swordOffer_Math().lastRemaining(n, k)
    }
    
    func findTheWinner2(_ n: Int, _ k: Int) -> Int {
        var queue = [Int]()
        queue.append(contentsOf: 1...n)
        while queue.count > 1 {
            for _ in 1..<k {
    ...
    func trap(_ height: [Int]) -> Int {
        ...
        var stack = [Int]()
        // 遍历数组进行处理
        for (curInd, curH) in height.enumerated() {
            // 如果栈不空并且当前高度大于栈顶下标所在高度，就可以一直循环出栈处理
            while !stack.isEmpty && curH > height[stack.last!] {
                // 出栈，并且取出栈顶下标所在水柱的高度
                let topHeight = height[stack.removeLast()]
                // 如果此时栈空，说明左边没有挡板，终止处理
                if stack.isEmpty {
                    break
                }
                // 计算当前元素和栈顶元素的距离，准备进行填充操作
                // 大多数情况相差1，如果是连续多个平地，则会大于1
                let distance = curInd - stack.last! - 1
                // 找出界定高度，栈顶的条形块被当前条形块和栈的前一个条形块界定
                // 其实就是找出凹槽
                let bounded_height = min(curH, height[stack.last!]) - topHeight
                print(curInd, stack.last!,distance,  bounded_height, stack.count)
                // 累加积水量
